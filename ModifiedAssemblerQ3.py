...
def floating(num):
    ...
    while(s>0):
        ...
        if(i>(8-l)):
            error=True
            print("Error on line number",line_no,"Invalid Immediate")
            break
...
def type_b(line):
    ...
    if(not error):
        ...
        if '$' not in words[2]:
            ...
            print("Error in line number",line_no, "Invalid Syntax")
    if(not error):
        if float(words[2][1:])>255 or float(words[2][1:])<0:
            error=True
            print("Error in line number ",line_no,"Inavlid immediate")
    ...
    if n.count('.')>0:
        ...
        if n[-1]!='.':
            ...
            if st.count('.')>0:
                error=True
                print("Error in line number",line_no,"Inavlid immediate")
            if not st.isdigit() and not error:
                error=True
                print("Error in line number",line_no,"Inavlid immediate")
        else:
            error=True
                ...
                if len(colon)!=0:
                    if colon[0]=='add':
                        ...
                        if not error:
                            ...
                            r2=colon[2]
                            r3=colon[3]
                            s="00000"+"00"+regaddress[r1]+regaddress[r2]+regaddress[r3]
                            check.append(s)
                    elif colon[0]=='subf':
                        type_a(lines[i])
                        if error:
                        ...
                        if not error:
                            ...
                            r2=colon[2]
                            r3=colon[3]
                            s="00001"+"00"+regaddress[r1]+regaddress[r2]+regaddress[r3]
                            check.append(s)
                    elif colon[0]=='movf':
                        type_b_f(lines[i])
                        if error:
                        ...
                        if not error:
                            r1=colon[1]
                            decimel=colon[2][1:]
                            r3=floating(float(decimel))
                            s="00010"+regaddress[r1]+r3
                            check.append(s)
                    else:
                        error=True
